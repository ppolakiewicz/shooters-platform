...
class TrainingEnrollmentUseCasesSpec extends Specification {
    ...
    def "creates and lists training enrollments for the owner"() {
        when: "The instructor creates a training enrollment template"
            def enrollment = booking.createTrainingEnrollment.create(owner, " Basic pistol ", " Safety and stance ", location(), 8, 2, 90)
    ...
    def "lists only the requested owner's training enrollments"() {
        given: "Two owners have enrollment templates"
            def ownerEnrollment = booking.createTrainingEnrollment.create(owner, "Basic pistol", "", location(), 8, 2, 90)
            booking.createTrainingEnrollment.create(otherOwner, "Other pistol", "", location(), 8, 2, 90)
    ...
    def "updates training enrollment owned by the instructor"() {
        given: "A training enrollment exists"
            def enrollment = booking.createTrainingEnrollment.create(owner, "Basic pistol", "", location(), 8, 2, 90)

        when: "The owner updates it"
            def updated = booking.updateTrainingEnrollment.update(owner, enrollment.id(), "Advanced pistol", "Updated", location(), 10, 3, 120)
    ...
    def "rejects update by a different owner"() {
        ...
        when: "Another owner tries to update it"
            booking.updateTrainingEnrollment.update(otherOwner, enrollment.id(), "Advanced pistol", "", location(), 10, 3, 120)
