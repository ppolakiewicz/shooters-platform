...
public class ReservationApiClient {
    ...
    public ResultActions reserve(MockHttpSession session, String body) throws Exception {
        var request = post("/api/bookings/reservations")
            .with(csrf())
    ...
    public ResultActions list(MockHttpSession session, UUID termId) throws Exception {
        return mockMvc.perform(get("/api/bookings/terms/{termId}/reservations", termId)
            .session(session));
    }
    ...
    public ResultActions cancelByInstructor(MockHttpSession session, UUID termId, UUID reservationId) throws Exception {
        return mockMvc.perform(post("/api/bookings/terms/{termId}/reservations/{reservationId}/cancel-by-instructor", termId, reservationId)
            .session(session)
            .with(csrf()));
    }

    public ResultActions expireWaitlistOffers(MockHttpSession session, UUID termId) throws Exception {
        return mockMvc.perform(post("/api/bookings/terms/{termId}/reservations/expire-waitlist-offers", termId)
            .session(session)
            .with(csrf()));
    }
