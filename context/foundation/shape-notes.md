...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "typ kontekstu"
      decision: "greenfield; użytkownik nadpisał automatyczne wykrycie brownfield"
    - topic: "główna persona"
      decision: "organizator i uczestnik są równorzędni w MVP"
    - topic: "główna kategoria bólu"
      decision: "mieszanka tarcia workflow, kosztu koordynacji i brakującej możliwości"
    - topic: "kluczowy wniosek"
      decision: "szablony kursów, aktywne powiadomienia listy rezerwowej i automatyczne potwierdzanie obecności przed sesją są centralne"
    - topic: "strategia auth"
      decision: "logowanie e-mailem i hasłem"
    - topic: "model ról"
      decision: "role Organizator i Uczestnik"
    - topic: "zakres MVP"
      decision: "główny przepływ MVP to publikowanie datowanych sesji z szablonu, rejestracja lub lista rezerwowa zależnie od pojemności, szablonowy e-mail przed sesją i potwierdzenie obecności; przepływy promocji z listy rezerwowej są drugorzędne"
    - topic: "budżet czasu"
      decision: "3 tygodnie pracy po godzinach dla głównego przepływu MVP"
    - topic: "typ produktu"
      decision: "webowa aplikacja mobile-first komunikująca się z aplikacją backendową"
    - topic: "docelowa skala"
      decision: "dziesiątki do stu użytkowników; reguła pojemności sesji i listy rezerwowej w MVP nie zmienia się przy skali 100x"
    - topic: "nie-cele"
      decision: "brak modułu płatności albo billingowego; brak zaawansowanego zarządzania administracyjnego"
  frs_drafted: 8
...
Pomysł wyjściowy: stworzyłem aplikację do ogłaszania terminów kursów strzeleckich i zarządzania zapisami na te kursy.

## Wizja I Opis Problemu

Organizatorzy szkoleń powtarzają pracę związaną z przygotowaniem kursów i potwierdzaniem udziału, a uczestnicy chcący zapisać się na szkolenie muszą ręcznie śledzić dostępność, gdy sesja jest pełna. Problem pojawia się, gdy organizatorzy tworzą nową datowaną sesję szkoleniową od zera zamiast użyć szablonu, gdy uczestnicy nie mogą dołączyć do listy oczekujących i otrzymać powiadomienia po zwolnieniu miejsca oraz gdy instruktorzy muszą ręcznie wysyłać e-maile przed każdą sesją, aby potwierdzić obecność.

Dzisiejszy koszt to wielokrotne wpisywanie opisu kursu, gdy zmienia się tylko data, utracone rejestracje, ręczna praca e-mailowa przed sesją i niepewność uczestników wokół dostępności.

Kluczowy wniosek jest taki, że szablony kursów, aktywne powiadomienia listy rezerwowej i automatyczne potwierdzanie obecności przed sesją razem usuwają obecną pracę ręczną i ryzyko utraconych rejestracji.
...
- Typ produktu: webowa aplikacja mobile-first komunikująca się z aplikacją backendową.
- Docelowa skala: dziesiątki do stu użytkowników.
- Notatka o skali: przy skali 100x reguła pojemności sesji i listy rezerwowej w MVP nie zmienia się.
- Harmonogram: brak twardego deadline'u; praca po godzinach; budżet MVP to 3 tygodnie.
...
- Logika biznesowa: obecna.
- Artefakty projektu: obecne.
- Ujęcie czasu i kosztu: obecne; budżet MVP 3 tygodnie.
- Poza zakresem: obecne.
- Zachowane zachowanie: n/a dla greenfield.
